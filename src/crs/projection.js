...
const unitsPerLonDegree = worldWidth / 360;
const unitsPerLatRadian = worldHeight / (2 * Math.PI);
...
export function project({lat, lng}) {
  const x = origin.x + lng * unitsPerLonDegree;
...
export function unproject({x, y}) {
  const lng = (x - origin.x) / unitsPerLonDegree;
  const latRadians = (y - origin.y) / -unitsPerLatRadian;
